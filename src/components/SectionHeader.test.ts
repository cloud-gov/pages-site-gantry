...
describe('SectionHeader', () => {
  ...
  const renderHTML = async (
    ...
    const res = await container.renderToString(SectionHeader, { props, slots }) as unknown;
  ...
  it('renders the correct heading level when provided', async () => {
    const html = await renderHTML({ headingLevel: 'h4' }, { default: 'Level 4' });
  ...
  it('falls back to h2 for an invalid headingLevel', async () => {
    const html = await renderHTML({ headingLevel: 'banana' as any }, { default: 'Fallback' });
  ...
  it('renders without requiring a class when none is provided', async () => {
    const html = await renderHTML({ headingLevel: 'h5' }, { default: 'No Class' });
