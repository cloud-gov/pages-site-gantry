...
export function organizeNavItems(items: NavItem[]): NavItem[] {
  ...
  if (categorized.general) {
    organized.push(...categorized.general.sort((a, b) => (a.order || 0) - (b.order || 0)));
  }
 ...
 */
export function findActiveNavItem(items: NavItem[], currentPath: string): NavItem | null {
 ...
 */
export function createBreadcrumbs(items: NavItem[], currentPath: string): NavItem[] {
  ...
  return menuItems
    .map((item: any) => {
      ...
      return {
        ...
        href: href,
        order: item.order || 0,
        children: item.subitems ? item.subitems
          ...
          .map((subitem: any) => {
            ...
            } else if (subitem.blockType === "externalLink" && subitem.url) {
              subHref = subitem.url;
            }
